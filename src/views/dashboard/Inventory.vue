...
const validate = (): boolean => {
  ...
  if (Number(f.stock) < 0) formErrors.value.stock = 'Stock cannot be negative';
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <!-- Tabs -->
          <div class="flex gap-1 px-6 py-3 border-b border-slate-100 dark:border-slate-800 overflow-x-auto shrink-0">
            <button
              v-for="tab in tabs"
              :key="tab.id"
              :class="[
          ...
          <div :class="['flex-1 overflow-y-auto p-6', isFormShaking ? 'animate-shake' : '']">
            ...
            <div v-show="activeTab === 'pricing'" class="space-y-5">
              ...
              <div v-if="profitMargin !== null" class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <p v-else-if="Number(profitMargin) < 10" class="text-xs text-amber-500 font-medium mt-1">
                  Low margin — consider raising the price
                </p>
              </div>
            ...
            <div v-show="activeTab === 'stock'" class="space-y-5">
              ...
              <div v-if="form.hasVariants" class="space-y-3">
                <div class="flex items-center justify-between">
                  <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Variants</p>
